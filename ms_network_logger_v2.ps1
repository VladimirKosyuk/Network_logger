...
# For all prodution MS 2012 R2 servers and above simultaneously collects network established connections(every 10 secodns) and active connections(realtime) to csv files during 12 hours and moves them to SMB-share. Outputs progress to CLI.
...
-Я не умею передавать в invoke переменные извне, потому для изменения таймера надо менять 108 строку
...
#parse data
    foreach ($pc in $List_SRV) 
        ...
        get-winevent -filterhashtable @{path=$Etl;Id='1033'} -Oldest |
        where-object {$_.ProcessId -inotmatch "0"}|
        select @{Name = 'Route'; Expression = {(($_.message) -replace "^.+([(])|([)])|(connect).+$") }}, @{Name = 'PID'; Expression = {($_.message) -replace "(TCP).+(PID = )|[.]"}},TimeCreated|
